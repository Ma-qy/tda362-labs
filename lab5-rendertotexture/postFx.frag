...
void main()
{
	...
	case 6:
		...
		break;
	case 7:
		fragmentColor = motionblur(frameBufferTexture, vec2(0.001,0), gl_FragCoord.xy, vec2(0., 5.));
		break;
	case 8:
...
vec3 toSepiaTone(vec3 rgbSample)
{
	...
	vec3 yiq = vec3(dot(yiqTransform0, rgbSample), 0.5, 0.5);
...
vec2 mushrooms(vec2 inCoord)
{
	return inCoord + vec2(cos(time * 4.3127 + inCoord.y / 9.0) * 15.0, 0.0);
}
