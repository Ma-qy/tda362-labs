...
layout(location = 0) out vec4 outColor;  // Output color to the fragment shader
...
void main() {
    uint idx = gl_VertexID; // Using gl_VertexID to index into the particles array
    Particle p = particles[idx];

    gl_Position = modelViewProjectionMatrix * p.position;  // Assign position
    gl_PointSize = p.size;     // Set the size for GL_POINTS rendering
    outColor = p.color;        // Pass the color to the fragment shader
}
